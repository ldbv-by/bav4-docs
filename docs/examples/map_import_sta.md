...
<div class="example">
	<h3>OGC SensorThings API (STA)</h3>
	<p>This example shows how to import an OGC SensorThings API and listen to FeatureInfo events.</p>
	<p>
	The STA ist imported by its URL. The desired ObservedProperty of the STA is referenced by the expression following the double-pipe. In our example, therefore <code>https://iot.hamburg.de/v1.1||5652</code>.</p>
	<div id="dfjc52vc" class="output"><bayern-atlas l="GEORESOURCE_WEB_GRAY" c="9.99,53.52" z="8"></bayern-atlas></div>
	<div id="ksg3bna_desc">Click in on a feature of the STA...</div>
	...
	<script>
		document.querySelector('#dfjc52vc bayern-atlas').addEventListener('baLoad', (event) => {
			...
			baMap.addLayer(
				'https://iot.hamburg.de/v1.1||5652', {cluster: true}
			);
