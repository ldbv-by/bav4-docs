<h1>BayernAtlas WebComponent Examples</h1>
<h2>Quick Start</h2>

<!-- Quick Start Example -->
<div class="example">
	<h3>1. Setup the Map</h3>
	<div class="output"><bayern-atlas l="GEORESOURCE_AERIAL" z="10" c="11,48"></bayern-atlas></div>
...
<!-- Quick Start Example - Programmatically -->
<div class="example">
	<h3>2. Setup Map programmatically</h3>
	<div id="skd63" class="output"><bayern-atlas></bayern-atlas></div>
	<script>
		document.querySelector('#skd63 bayern-atlas').addEventListener('baLoad', (event) => {
			const ba = event.target;
			const { GEORESOURCE_AERIAL } = ba;
			ba.addLayer(GEORESOURCE_AERIAL);
			ba.modifyView({ zoom: 10, center: [11, 48] });
		});
	</script>
	<h4>HTML:</h4>
	<pre><code class="language-html">&lt;bayern-atlas l=&quot;&quot;&gt;&lt;/bayern-atlas&gt;</code></pre>
	<h4>JavaScript:</h4>
	<pre><code class="language-javascript">document.querySelector('bayern-atlas')
...
<b></b>aMap.addLayer(dataAsKml, { zoomToExtent: true, modifiable: true });  // make the layer modifiable: Note: A modifiable layer must meet the following expectations: Its data must have the format `KML` and must previously be created by the BayernAtlas
baMap.addEventListener("baGeometryChange",(event) => {
